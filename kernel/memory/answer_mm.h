...
void kern_page_free(void* ptr) {
	...
	for (int i=0;i<INIT_MAX_PAGES;i++)
		if ((uint64)(&kernel_page_initialized[i*BUDDY_PAGE_SIZE])==(uint64)ptr) flag=true;
	for (int i=allocated_page;i<INIT_MAX_PAGES;i++)
		if (kernel_page_available[i]==(uint64)ptr) flag=false;
    if (!flag) {
